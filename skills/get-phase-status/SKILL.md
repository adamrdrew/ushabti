...
!`if [ -d .ushabti/phases ] && [ "$(ls -A .ushabti/phases 2>/dev/null)" ]; then for dir in .ushabti/phases/*/; do name=$(basename "$dir"); status=$(grep "^  status:" "$dir/progress.yaml" 2>/dev/null | awk '{print $2}'); impl=$(grep -c "implemented: true" "$dir/progress.yaml" 2>/dev/null || echo 0); total=$(grep -c "implemented:" "$dir/progress.yaml" 2>/dev/null || echo 0); echo "$name: $status ($impl/$total steps)"; done; else echo "No phases exist yet"; fi`
